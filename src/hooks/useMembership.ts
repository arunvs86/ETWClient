 ...
 * Safe membership hook:
 * - Does NOT fetch when logged out (or while auth state is loading)
 * - Light retry for flakes
 * - Cache is scoped per user to avoid stale reuse after account switch
 */
export function useMembership() {
  const { user, loading: authLoading } = useAuth();
  const isAuthed = !!user;
  ...
  const q = useQuery<Membership | undefined>({
    queryKey: ["me", "membership", userKey],
    queryFn: async () => {
      const data = await getMyMembership();
      return data ?? undefined;
    },
    enabled: isAuthed && !authLoading, // only when signed in and auth ready
    retry: 1,                          // avoid loops but tolerate a transient blip
    staleTime: 5 * 60 * 1000,
  });

  const membership = q.data;
  const status = membership?.status;
  const isActive = status === "active" || status === "trialing"; // adjust if trialing is not active for you
