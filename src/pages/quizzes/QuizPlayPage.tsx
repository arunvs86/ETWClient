...
// //   // On first mount: resume if attemptId in state or localStorage; else start fresh
// //   useEffect(() => {
// //     (async () => {
// //       try {
// //         if (!attemptId) {
...
// //         }
// //         if (attemptId || localStorage.getItem(`quizAttempt:${slug}`)) {
// //           const id = attemptId || (localStorage.getItem(`quizAttempt:${slug}`) as string)
// //           const resp = await getAttempt(id)
// //           if (resp.attempt.status === 'submitted') {
// //             nav(`/quizzes/attempts/${id}`, { replace: true })
// //             return
// //           }
...
// //           setExpiresAt(null) // server may not send; timer derives from startedAt+limit
// //         } else {
...
// //   // autosave
// //   const answerArray = useMemo(() => Object.entries(answers).map(([questionId, v]) => {
...
// //             <div className="mb-2 text-xs text-gray-500">{idx + 1} / {questions.length} • {q.points} pt{q.points !== 1 ? 's' : ''}</div>
...
// // function StatusBadge({ status, lastSavedAt }: { status: ReturnType<typeof useAttemptAutosave>['status']; lastSavedAt: Date | null }) {
// //   const map: Record<string, { cls: string; label: string }> = {
// //     idle: { cls: 'bg-gray-100 text-gray-700', label: 'Idle' },
// //     saving: { cls: 'bg-amber-100 text-amber-800', label: 'Saving…' },
// //     saved: { cls: 'bg-emerald-100 text-emerald-800', label: lastSavedAt ? `Saved • ${lastSavedAt.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}` : 'Saved' },
...
//   const { slug = '' } = useParams()
//   const nav = useNavigate()
//   const state = (history.state?.usr || {}) as NavState // stays same as your previous `loc.state`
...
//       const resp = await submitAttempt(attemptId)
//       localStorage.removeItem(`quizAttempt:${slug}`)
//       // UX: ensure we land at the top of the results page
//       window.scrollTo({ top: 0, behavior: 'auto' })
//       nav(`/quizzes/attempts/${attemptId}`, { replace: true, state: { data: resp } })
//     } catch (e: any) {
//       setErr(e?.response?.data?.message || 'Submit failed')
