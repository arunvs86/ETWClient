...
//       <div className="relative w-full sm:w-96">
//         <input
//           className="w-full h-10 rounded-md border border-gray-300 px-3 pr-8 focus:outline-none focus:ring-2 focus:ring-primary/30"
//           placeholder="Search…"
//           value={input}
//           onChange={(e) => setInput(e.target.value)}
//         />
...
//               >
//                 <div className="font-medium group-hover:underline">{q.title}</div>
//                 <div className="mt-1 line-clamp-2 text-sm text-gray-600">{q.description}</div>
//                 <div className="mt-2 flex flex-wrap gap-2 text-xs text-gray-500">
//                   <span className="rounded bg-gray-100 px-2 py-0.5">{q.questionCount} Qs</span>
//                   <span className="rounded bg-gray-100 px-2 py-0.5">{q.totalPoints} pts</span>
//                   <span className="rounded bg-gray-100 px-2 py-0.5">Pass {q.passPercent}%</span>
//                 </div>
//               </Link>
//             ))}
//           </div>
...
// src/pages/quizzes/PublicQuizListPage.tsx
import { useEffect, useMemo, useState } from 'react'
import { Link, useSearchParams } from 'react-router-dom'
import { useQuery } from '@tanstack/react-query'
import { listPublishedQuizzes, type QuizListResp } from '@/lib/publicQuizzes.api'
import { Loader2, Search, X, BookOpenCheck, AlertCircle } from 'lucide-react'
    ...
    <div className="mx-auto max-w-6xl p-4 sm:p-6 space-y-5">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="w-full sm:w-[28rem]">
          <div className="relative">
            ...
            <input
              className="h-10 w-full rounded-xl border border-gray-300 pl-10 pr-20 text-sm outline-none transition focus:ring-2 focus:ring-primary/30"
      ...
      {isLoading ? (
        <>
          ...
          <div className="mt-3 h-8 w-40 animate-pulse rounded-md bg-gray-100" />
        </>
      ) : isError ? (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="font-medium leading-5 group-hover:underline">
                      {qz.title}
                    </div>
