...
//   const ent = qEnt.data as { hasAccess?: boolean; canJoinNow?: boolean; source?: string } | undefined;
//   const hasAccess = !!ent?.hasAccess;
//   const canJoinNow = !!ent?.canJoinNow;
...
// ) : needAuth ? (
//   <Button onClick={() => nav(`/login?next=/live/${session!.id}`)} full>
//     Login to continue
//   </Button>
// ) : qEnt.isLoading ? (
...
//         You have access. The join link will be enabled {JOIN_WINDOW_MIN} minutes before the start time.
//       </div>
//       {ent?.source && (
//         <div className="mt-1 text-[11px] text-gray-600">
//           Access via {ent.source}.
//         </div>
//       )}
//     </>
//   )
...
//     {import.meta.env.DEV &&
//       import.meta.env.VITE_LIVESESSIONS_DEV_FAKE_PURCHASE === 'true' && (
//         <button onClick={onDevBuy} className="text-xs underline text-gray-600">
...
function InformationIcon() {
  return (
    <svg aria-hidden viewBox="0 0 24 24" className="h-4 w-4 text-gray-600">
      <path
        d="M12 22a10 10 0 1 1 0-20 10 10 0 0 1 0 20zM11 10h2v7h-2v-7zm0-3h2v2h-2V7z"
        fill="currentColor"
      />
    </svg>
  );
...
function useJoinWindow(session?: PublicLiveSession) {
  ...
  const openFrom = start
    ? new Date(start.getTime() - JOIN_WINDOW_MIN * 60 * 1000)
    : null;
  const joinOpen = !!(
    start &&
    end &&
    now >= openFrom! &&
    now <= new Date(end.getTime() + 5 * 60 * 1000)
  );
  const startsIn = useMemo(() => {
    if (!start) return '';
    if (now >= start) return '';
...
function HeaderBlock({
  ...
  if (loading)
    return (
      <div className="h-28 rounded-2xl bg-gradient-to-r from-indigo-50 to-violet-50 animate-pulse" />
    );
        ...
        <div className="min-w-0 flex-1">
          {error ? (
            ...
            <>
              ...
              <h1 className="mt-2 text-2xl md:text-3xl font-semibold text-gray-900">
                {session?.title}
              </h1>
              {session?.description && (
...
export default function PublicLiveSessionView() {
  ...
  const session = qSession.data;
  const { startsIn } = useJoinWindow(session);

  const needAuth = !loading && !user;

  // new entitlement shape: { hasAccess, canJoinNow, source }
  const ent = qEnt.data as
    | { hasAccess?: boolean; canJoinNow?: boolean; source?: string }
    | undefined;
  const hasAccess = !!ent?.hasAccess;
  const canJoinNow = !!ent?.canJoinNow;
    ...
    try {
      ...
      alert(
        e?.response?.data?.error ||
          'Checkout failed (is the endpoint implemented?)',
      );
    }
  }
  ...
  useEffect(() => {
    if (pollEntitlement && !pollUntil)
      setPollUntil(Date.now() + 60_000);
  }, [pollEntitlement, pollUntil]);

  useEffect(() => {
    if (!pollEntitlement) return;
    const i = setInterval(() => {
      if (pollUntil && Date.now() > pollUntil) {
        setPollEntitlement(false);
        clearInterval(i);
        return;
      }
      qc.invalidateQueries({ queryKey: ['liveSessionEnt', id] });
  ...
  useEffect(() => {
    if (hasAccess) setPollEntitlement(false);
  }, [hasAccess]);
                ...
                <div className="mt-4 flex flex-wrap items-center gap-2 text-xs text-gray-700">
                  ...
                  {session && (
                    <LiveSessionBadges
                      ...
                      membersAccess={session.membersAccess}
                    />
                  )}
        ...
        <aside className="lg:sticky lg:top-6 h-min">
          <div className="rounded-2xl border bg-white p-5 shadow-sm">
            <div className="flex items-baseline justify-between">
              <div className="text-3xl font-bold text-primary">
                {isLoading ? (
                  <span className="inline-block h-8 w-28 rounded bg-gray-100 animate-pulse" />
                ) : (
                  priceLabel
                )}
              </div>
            ...
            <div className="mt-4">
              {isLoading ? (
                ...
                <Button
                  onClick={() =>
                    nav(`/login?next=/live/${session!.id}`)
                  }
                  full
                >
                  Login to continue
                ...
                canJoinNow ? (
                  ...
                  <>
                    <Button
                      disabled
                      full
                      title={`Link enables ${JOIN_WINDOW_MIN} min before start`}
                    >
                      Join (enabled {JOIN_WINDOW_MIN} min before start)
                    </Button>
                    <div className="mt-2 text-xs text-amber-700">
                      You have access. The join link will be enabled{' '}
                      {JOIN_WINDOW_MIN} minutes before the start time.
                    </div>
                    {ent?.source && (
                      <div className="mt-1 text-[11px] text-gray-600">
                        Access via {ent.source}.
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {import.meta.env.DEV &&
                    import.meta.env
                      .VITE_LIVESESSIONS_DEV_FAKE_PURCHASE ===
                      'true' && (
                      <button
                        onClick={onDevBuy}
            ...
            <ul className="mt-4 space-y-1 text-xs text-gray-600">
              <li>
                • Join opens about {JOIN_WINDOW_MIN} minutes before
                start time.
              </li>
              <li>• You’ll join in a new tab.</li>
            </ul>
