...
import {
  ...
  restoreCourse,
  deleteCourse,
  upsertSingleLesson, // <-- make sure this exists in your API file (step 2 below)
...
export default function CourseEditorPage() {
  ...
  const [thumbnail, setThumbnail] = useState<string>(''); // can be URL or data: URL
  ...
  useEffect(() => {
    ...
    // YouTube kept blank here; you can paste to overwrite the single lesson when saving
  }, [course]);

  // -------- Mutations (publish etc) --------
  ...
  // -------- Save all changes (basics + lesson + pricing) --------
  async function saveAll(e?: React.FormEvent) {
    ...
    try {
      ...
      await updateCourseBasics(id!, {
        ...
        thumbnail: thumbnail, // can be URL or data URL (temporary)
        promoVideoUrl: '',    // optional
      });

      // 2) Single YouTube lesson (only if provided)
      if (youtubeUrl.trim()) {
  ...
  // -------- Drag & drop for thumbnail (stores as data URL or lets you paste URL) --------
  ...
  async function onDrop(e: React.DragEvent) {
    ...
    reader.onload = () => {
      ...
      setThumbnail(dataUrl); // NOTE: stored as data URL; replace with your uploader later
      setErr(null);
    };
  ...
  return (
    <form onSubmit={saveAll} className="mx-auto max-w-5xl p-4 sm:p-6 space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-[1.25fr,0.9fr]">
        ...
        <div className="space-y-4 rounded-md border bg-white p-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <label className="text-sm">
              Duration (sec)
              <input className="mt-1 w-full h-10 rounded-md border px-3"
                     type="number" min={0} value={String(durationSec)}
        ...
        {/* Right column: thumbnail drag & drop */}
          ...
          >
            ...
            <div className="mt-2 grid gap-2">
              ...
              <p className="text-xs text-gray-500">
                Tip: Drag & drop an image, or paste an image URL. (Data URL is stored if you drop a file — replace with a real uploader later.)
              </p>
            </div>
          </div>
