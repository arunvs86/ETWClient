...
import {
  listMyInstructorResources,
  type MyResourcesResponse,
  type MyResourceListItem,
} from '@/lib/instructorResources.api'

function useQueryParams() {
  const [sp, setSp] = useSearchParams()
  const status =
    (sp.get('status') as 'draft' | 'published' | 'archived' | null) ?? null
  const q = sp.get('q') ?? ''
...
function StatusChip({ s }: { s: MyResourceListItem['status'] }) {
  const styles: Record<MyResourceListItem['status'], string> = {
  ...
  return (
    <span
      className={`inline-flex items-center rounded px-2 py-0.5 text-xs font-medium ${styles[s]}`}
    >
      {label}
    </span>
  )
...
export default function MyResourcesPage() {
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      if (input !== q) set({ q: input })
    }, 300)
    return () => clearTimeout(t)
  }, [input]) // eslint-disable-line

  const queryKey = useMemo(
    () => ['myResources', { status, q, page, limit }],
    [status, q, page, limit]
  )

  const { data, isLoading, isError, error, refetch, isFetching } =
    useQuery<MyResourcesResponse>({
      queryKey,
      queryFn: () =>
        listMyInstructorResources({ status: status || undefined, q, page, limit }),
      staleTime: 15_000,
    ...
    <div className="mx-auto max-w-6xl p-4 sm:p-6 space-y-4">
          ...
          {(['all', 'draft', 'published', 'archived'] as const).map((tab) => {
            ...
            return (
              <button
                ...
                } ${tab !== 'archived' ? 'border-r border-gray-200' : ''}`}
              >
      ...
      {isLoading ? (
        ...
        <div className="rounded-md border border-red-200 bg-red-50 p-4 text-red-700">
          Failed to load resources. {(error as any)?.message || ''}
          <div>
        ...
        <div className="rounded-md border border-gray-200 p-8 text-center">
          <div className="text-lg font-medium">No resources yet</div>
          <div className="mt-1 text-sm text-gray-600">
            Create your first resource to get started.
          </div>
          <Link
            to="/instructor/resources/new"
            className="mt-4 inline-flex h-10 items-center rounded-md bg-primary px-4 text-white hover:opacity-95"
          >
            + New resource
          </Link>
        </div>
        ...
        <>
                  ...
                  <tr key={r.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        {r.thumbnail ? (
                          <img src={r.thumbnail} alt="" className="h-10 w-16 rounded object-cover" />
                        ) : (
                    ...
                    <td className="px-4 py-3 text-sm text-gray-800">
                      {formatMoney(r.pricing)}
                      {r.pricing?.includedInMembership ? ' · In membership' : ''}
                    </td>
          ...
          <div className="grid grid-cols-1 gap-3 md:hidden">
            {items.map((r) => (
              ...
              >
                {r.thumbnail ? (
                  <img src={r.thumbnail} alt="" className="h-14 w-20 rounded object-cover" />
                ) : (
