...
export type PublicLiveSession = {
  ...
  timezone?: string;
  status: SessionStatus;
  visibility: 'public' | 'course' | 'private' | string;
  capacity?: number;
  pricing: LiveSessionPricing;
  membersAccess: MembersAccess;
  createdAt?: string;
  ...
  // Not currently in mapPublic(), but we'll leave it here
  zoom?: ZoomInput;
};
...
// We'll reuse this for create and update
export type BaseLiveSessionBody = {
  title: string;
  description?: string;
  startAt: string; // ISO
  endAt: string;   // ISO
  ...
  membersAccess?: MembersAccess;
  thumbnail?: string; // can be URL, can be data:image/... base64, or '' to clear
  zoom?: ZoomInput;
};

export type CreateLiveSessionBody = BaseLiveSessionBody;

export type EntitlementResponse = {
  hasAccess?: boolean;
  canJoin?: boolean; // legacy
  source?: 'free' | 'membership' | 'purchase';
  reason?: 'auth_required' | 'purchase_required' | 'purchase_required_even_for_members';
};
...
export async function getLiveSession(id: string) {
  const { data } = await api.get<PublicLiveSession>(
...
export async function joinLiveSession(id: string) {
  ...
  return data as { ok: boolean; url?: string; reason?: string };
}
...
export async function updateLiveSession(id: string, body: Partial<BaseLiveSessionBody>) {
  const { data } = await api.patch<PublicLiveSession>(
    `/live-sessions/${encodeURIComponent(id)}`,
    body
  );
  const s: any = data;
...
export async function deleteLiveSession(id: string) {
  const { data } = await api.delete<{ ok: boolean; error?: string }>(
    `/live-sessions/${encodeURIComponent(id)}`
  );
  return data;
}
...
/* ---------- DEV only ---------- */
export async function devPurchaseLiveSession(id: string) {
  const { data } = await api.post<{ ok: true; orderId: string }>(
    `/live-sessions/${encodeURIComponent(id)}/purchase`,
    {}
  );
  return data;
}
